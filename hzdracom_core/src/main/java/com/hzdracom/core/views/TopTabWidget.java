...
public class TopTabWidget extends LinearLayout {
	...
	@SuppressLint ("NewApi")
    public TopTabWidget(Context context, AttributeSet attrs, int defStyle) {
		...
		if (null == mLabels || mLabels.length <= 0) {
			try {
				...
				//Logger.e(TopTabWidget.class.getSimpleName() + " 出错");
			}
			a.recycle();
			return;
		}
	...
	public TopTabWidget(Context context, AttributeSet attrs) {
...
//		this.setBackgroundResource(R.color.common_bg);
		...
		for (int i = 0; i < size; i++) {
			...
			// 每个tab对应的layout
			final View view = inflater.inflate(R.layout.common_views_top_tab_item, null);

			final CheckedTextView itemName = (CheckedTextView) view
					.findViewById(R.id.item_name);
			itemName.setCompoundDrawablesWithIntrinsicBounds(null, null, null, context
          .getResources().getDrawable(R.drawable.red_line_selector));
			itemName.setText(mLabels[i]);
      ////红线
      //Drawable red_line = getResources().getDrawable(R.drawable.red_line_selector);
      //red_line.setBounds(0, 0, red_line.getMinimumWidth(), red_line.getMinimumHeight());
      //itemName.setCompoundDrawables(null, null, null, red_line);
			// 指示点TextView，如有版本更新需要显示
	 ...
	 *            是否显示，如果false，则都不显示
	 */
	public void setIndicateDisplay(Context context, int position, String number,
			boolean visible) {
...
//				Logger.d(mLabels[index] + " is selected...");
