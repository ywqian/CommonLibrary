...
public class TabWidget extends LinearLayout {
	...
	private int[] mDrawableIds = new int[] { R.drawable.tab_first_selector,
			R.drawable.tab_second_selector, R.drawable.tab_third_selector,
			R.drawable.tab_fourth_selector };
    ...
    public TabWidget(Context context, AttributeSet attrs, int defStyle) {
		...
		if (null == mLabels || mLabels.length <= 0) {
			try {
				...
				//Logger.e(TabWidget.class.getSimpleName() + " 出错");
			}
			a.recycle();
			return;
		}
...
//		this.setBackgroundResource(R.color.common_bg);
		...
		for (int i = 0; i < size; i++) {
			...
			// 每个tab对应的layout
			final View view = inflater.inflate(R.layout.common_views_tab_item, null);
